...
from definition_extraction.utils import constant
...
class Vocab(object):
    def __init__(self, filename, load=False, word_counter=None, threshold=0):
        if load:
            ...
            assert (
                word_counter is not None
            ), "word_counter is not provided for vocab creation."
            ...
            self.id2word = sorted(
                self.word_counter, key=lambda k: self.word_counter[k], reverse=True
            )
    ...
    def map(self, token_list):
        ...
        """
        return [
            self.word2id[w] if w in self.word2id else constant.VOCAB_UNK_ID
            for w in token_list
        ]
    ...
    def get_embeddings(self, word_vectors=None, dim=100):
        self.embeddings = (
            2 * constant.EMB_INIT_RANGE * np.random.rand(self.size, dim)
            - constant.EMB_INIT_RANGE
        )
        if word_vectors is not None:
            assert (
                len(list(word_vectors.values())[0]) == dim
