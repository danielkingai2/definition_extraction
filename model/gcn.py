...
from definition_extraction.model.tree import Tree, head_to_tree, tree_to_adj
...
class GCNClassifier(nn.Module):
    ...
    def forward(self, inputs):
        ...
        out = pool(outputs, masks.unsqueeze(2).bool(), type=pool_type)
        out = self.out_mlp(out)
        sent_logits = self.sent_classifier(out)

        terms_out = pool(
            F.softmax(outputs), terms.unsqueeze(2).byte().bool(), type=pool_type
        )
        defs_out = pool(
            F.softmax(outputs), defs.unsqueeze(2).byte().bool(), type=pool_type
        )
        term_def = (terms_out * defs_out).sum(1).mean()
        not_term_def = (
            (terms_out * defs_out[torch.randperm(terms_out.shape[0])]).sum(1).mean()
        )

        selections = self.selector(gcn_outputs)

        defs_out = (
            pool(outputs, defs.unsqueeze(2).bool(), type=pool_type)
...
def rnn_zero_state(
    batch_size, hidden_dim, num_layers, bidirectional=True, use_cuda=True
):
